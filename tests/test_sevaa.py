...
TOKEN = "TEST_AUTOMATION_TOKEN_VALUE"
...
class TestPaymentReconciliation(AionTest):
    ...
    def test_cli_reconcile_records_when_configured(self):
        # Deliberately a standalone `aion sevaa-reconcile` command, not wired
        # into resume.boot(): aion_core/resume.py is a protected path and
        # this task has no baseline override to touch it.
        import urllib.request
        ...
        try:
            exit_code = cli.main(["sevaa-reconcile"])
        finally:
            urllib.request.build_opener = original
        self.assertEqual(exit_code, 0)

    def test_cli_reconcile_is_a_quiet_noop_when_not_configured(self):
        import os
        from aion_core import cli, config
        config.secrets_file().write_text("")
        os.environ.pop(sevaa.AUTOMATION_TOKEN_NAME, None)
        self.assertEqual(cli.main(["sevaa-reconcile"]), 0)
