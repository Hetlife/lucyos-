...
Salvaged from claude/fable-deploy-setup-mc5nr6 (S-29). Ported rather than
imported wholesale: the original tests also asserted against aion_core.phone
(a phone-dashboard module) and an aion_core.reports.status() that listed
experiments inline. Neither exists on this tree and neither is in this
task's ALLOWED scope (aion_core/experiments.py, tests/test_experiments.py,
aion_core/cli.py only) -- porting them in would silently widen the task.
Everything that exercises experiments.py itself is kept.
"""
import json
import os
import unittest
...
from aion_core import experiments, metrics
