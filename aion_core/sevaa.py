...
def _read_secret(name: str) -> str | None:
    ...
    Matches aion_core.backup._read_secret's pattern -- this module has no
    dependency on it to avoid a cross-import for one helper."""
...
def secret() -> str | None:
    ...
    return _read_secret(SECRET_NAME) or os.environ.get(SECRET_NAME) or None
...
def base_url() -> str:
    return _read_secret(BASE_URL_ENV) or os.environ.get(BASE_URL_ENV, "").strip() \
...
def automation_token() -> str | None:
    return _read_secret(AUTOMATION_TOKEN_NAME) or os.environ.get(AUTOMATION_TOKEN_NAME) or None


def founder_token() -> str | None:
    return _read_secret(FOUNDER_TOKEN_NAME) or os.environ.get(FOUNDER_TOKEN_NAME) or None
