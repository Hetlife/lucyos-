...
from . import config, health, util, worker
...
def _systemd_available() -> bool:
    if not shutil.which("systemctl"):
        return False
    try:
        p = subprocess.run(["systemctl", "--user", "is-system-running"],
                           capture_output=True, text=True, timeout=8)
    except (OSError, subprocess.TimeoutExpired):
        return False
    # Any answer at all means a user manager responded; "degraded" still counts.
...
def render(result: dict) -> str:
    ...
    lines = [
        ...
        f"  systemd --user: {'yes' if caps['systemd_user'] else 'no'}",
        f"  OpenClaw: " + (
