...
def host_capabilities() -> dict:
    ...
    caps["systemd_user"] = _systemd_available()
    caps["python_ok"] = sys.version_info >= (3, 9)
    return caps


def _systemd_available() -> bool:
    # Keep platform knowledge behind the existing host adapter.  The adapter
    # deliberately reports a conservative native-timer fallback for unknown
    # platforms; known hosts have a supported user scheduler.
    return host.current().scheduler_kind() != "native-timer"
...
def render(result: dict) -> str:
    ...
    lines = [
        ...
        f"  user scheduler: {'yes' if caps['systemd_user'] else 'no'}",
        f"  OpenClaw: " + (
